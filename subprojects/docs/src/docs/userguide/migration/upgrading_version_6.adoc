...
A fix done to disambiguate projects https://github.com/gradle/gradle/issues/847[having the same name] in dependency resolution made it possible to start publishing projects with identical coordinates.
This could lead to invalid publications as multiple modules would overwrite each other.

Given the above, a duplicate identifier check was added to prevent invalid publications.
In some corner cases, the check can prove too eager and may need some action from the build author.
...
If your build fails with a message as above, you have two options:

* Make sure you configure both the `groupId` and `artifactId` in the publication to resolve ambiguities
* Disable this check if it is a false positive in your setup
** By adding `-Dorg.gradle.dependency.duplicate.project.detection=false` to the CLI
** Or by adding `systemProp.org.gradle.dependency.duplicate.project.detection=false` to `gradle.properties`
